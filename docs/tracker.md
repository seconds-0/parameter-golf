...
- Track F is trusted for launch/measurement, but the next work in sequence is now a focused trainer serialization bughunt: prove why live final metrics and saved artifacts diverge, add an in-trainer reload self-check, and only then restart `E03`/`E04`
