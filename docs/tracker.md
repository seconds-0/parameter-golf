...
- `E34c` is now complete and mildly positive but below the promote bar. On the same promoted WSD + asymmetric `(20,30)` + `E30` base and the same eager fallback, control `phase1_e34c_control_p1-20260321-004949-c4a83875` landed at postquant `1.41100907`, prequant `1.40944965`, `qgap=0.00155942`, and `242.95 ms/step`, while the NorMuon candidate `phase1_e34c_normuon_p1-20260321-010303-d12f0bd7` improved to postquant `1.40981103`, prequant `1.40835540`, and `qgap=0.00145563`, for `Δpq=-0.00119804`, `Δpre=-0.00109425`, and `Δqgap=-0.00010379`, while step time regressed modestly to `247.54 ms` (`+1.89%`). That is a real but too-small gain, so the active base remains WSD + asymmetric `(20,30)` + `E30` and the right next diversification branch is `E24a`.
- Post-mortem coverage now exists for every completed non-baseline experiment in the live tracker through `E34c`: `E03`, `E04`, `E23`, `E27`, `E28`, `E30`, `E32`, `E34a`, `E34c`, and `E35` all have closeout reviews in [docs/postmortems/](./postmortems/README.md), and none currently need reopening.
- Static exporter-only tuning of quantization knobs is therefore exhausted for this checkpoint. However, `E36a/b` eval-time softcap sub-sweeps are a new zero-cost eval-only experiment family that tests whether using a different softcap at eval time (vs training time) improves post-roundtrip val_bpb. `E36a` sweeps symmetric scale, `E36b` does asymmetric local search around the active `(20,30)` base, and `E36c` (plain temperature scaling `logits/T`) is deferred until code support is added. Literature on post-hoc calibration (Guo et al. 2017, Wang et al. 2020) suggests meaningful NLL improvement is possible. Infrastructure already supports softcap overrides via `export_eval.py --set-env`.
- The best overall next cheap tranche is `E36a/b` eval-time softcap sub-sweeps (zero training cost). For training-aware Track B work, the next concrete branch is `E24a` fixed L2 weight decay for export retention. The working hypothesis is still the same: our biggest wins so far have come from improving training efficiency or optimizer quality rather than exporter-only knob sweeps, but `E34c` did not clear the bar strongly enough to justify staying in the Muon lane by default.
- `E34c` still matters as evidence: NorMuon did improve both prequant and post-roundtrip quality slightly, so the optimizer lane is not dead, but it did not earn an immediate follow-up over the cleaner Track B diversification branch.
- If `E24a` is flat and we want to revisit optimizer internals, `E34b` or a deeper `E34c` follow-up remain viable later. Right now the disciplined move is to diversify rather than stack another below-threshold Muon change.
