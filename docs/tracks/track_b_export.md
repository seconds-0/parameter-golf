...
- **E24a**: Fixed L2 weight decay — standard L2 weight decay on 2D matrices, from [Q Labs](../references/qlabs_10x_data_efficiency.md). **Complete, killed early** — the first nonzero point (`wd=0.1`) catastrophically regressed both prequant and post-roundtrip quality, so the sweep was stopped without spending time on `0.5` or `1.0`.
...
## Status
Unblocked, but the static exporter-only stars are now complete and flat and the first simple training-aware regularizer branch also failed cleanly. `P-02` is complete, `E02` passed on `baseline_repro-20260319-093041-82dade88`, and `X-05` is trusted on both a fresh P1 proof run and the actual trusted `E02` baseline checkpoint. `E03` and `E04` both finished with `no win`, `E23` export-time EMA was killed, and `E24a` fixed L2 weight decay was then killed early after the first nonzero point. Detailed closeout reviews: [E03](../postmortems/e03_exporter_clip_star.md), [E04](../postmortems/e04_keep_float_threshold.md), [E24a](../postmortems/e24a_fixed_weight_decay.md).
...
- `E24a` then answered the obvious “cheap standard regularizer” question decisively: matched Runpod control `phase1_e24a_control_p1-20260321-020005-afea0bac` landed at postquant `1.41965284`, while `phase1_e24a_wd01_p1-20260321-020914-e9d70fa7` with `FIXED_WEIGHT_DECAY=0.1` regressed catastrophically to `1.70560576`
- The failure is not an export-only trade. Prequant also collapsed from `1.41811530` to `1.66554339`, and `qgap` exploded from `0.00153754` to `0.04006236`, which means this branch is conceptually wrong for the current promoted stack rather than merely over-tuned
- Because `wd=0.1` already fails the Track B kill rule by a huge margin, the sweep was stopped without running `0.5` or `1.0`; those points are overwhelmingly more likely to be worse than better
- The next Track B-specific branches are therefore `E24b`, `E33`, or `E13` one at a time, but the broader repo should do the first real `8xH100` calibration before opening another new regularizer branch
