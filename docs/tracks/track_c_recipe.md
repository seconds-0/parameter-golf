...
## Status
Partially unblocked. Tokenizer-dependent recipe work `E10`-`E12` still depends on `E09` (tokenizer winner selected), while the cheap side experiments `E30` and `E34` remain unblocked by `E02`. `E23` and `E35` are complete and killed, `E34a` is complete and neutral / no promote, and `E32`, `E28`, and `E30` are now complete and promoted. The active Track C base is WSD plus asymmetric softcap `(20,30)` plus the `E30` early-small-batch schedule. Detailed closeout reviews: [E23](../postmortems/e23_ema_export.md), [E28](../postmortems/e28_asymmetric_logit_rescale.md), [E30](../postmortems/e30_batch_schedule.md), [E32](../postmortems/e32_wsd_schedule.md), [E34a](../postmortems/e34a_polar_express.md), [E35](../postmortems/e35_cooldown_beta2.md).

The recommended next Track C order is:
- `E34c` NorMuon on top of the promoted WSD + asymmetric `(20,30)` + `E30` base
- `E34a` came back effectively neutral, so `E34c` is now the strongest same-theme follow-up because it targets optimizer quality through a different mechanism and should compose more naturally than an immediate architecture pivot
- if `E34c` is flat, the best diversification branch is usually `E24a` fixed L2 weight decay before spending effort on heavier structural changes
