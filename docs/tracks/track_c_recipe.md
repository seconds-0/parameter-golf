...
- `CAL-01` therefore makes `E30` the leading suspect. The candidate curve peaked at about step `6200`, almost exactly where the `E30` batch schedule transitions from `131072` to `524288` tokens, while WSD is still in its stable max-LR phase. That interaction is now the most plausible late-run failure mode and needs explicit full-run decomposition
...
- The right lesson is now "proxy promotions are not enough by themselves." `E34c` did not clear the `-0.003` promote bar, `E24a` then failed catastrophically, and `CAL-01` showed that the promoted proxy stack underperforms the trusted baseline throughout full training. So the next branch should be full-run decomposition rather than another same-family tweak by default
