...
## Status
Trusted for launch/measurement on both the training path and standalone saved-artifact replay path, with one current caveat: fresh-host `torch.compile` is unstable on Vast right now, so the `E30` tranche was run under a matched `TORCHDYNAMO_DISABLE=1` eager fallback. `P-01`, `P-02`, `P-03`, `P-04`, `P-05`, `P-06`, `X-01`, `X-02`, `X-05`, `E00`, `E01`, `E02`, `E27`, `E28`, `E30`, `E32`, and `E35` are done, and the first baseline exporter sweeps `E03`/`E04` are also complete and flat. There is no urgent measurement-integrity blocker right now, but there is a fresh-host compiled-path reliability issue to revisit. The broader repo next tranche is `E34a` on top of the promoted WSD + asymmetric `(20,30)` + `E30` base, with `E24a` as the next Track B-specific branch if we want to stay on export retention after that. Detailed closeout review for the Track F experiment branch: [E27](../postmortems/e27_doc_aligned_batching.md).
