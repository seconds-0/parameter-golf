...
## Status
Trusted for launch/measurement on both the training path and standalone saved-artifact replay path, with two current caveats: fresh-host `torch.compile` is unstable on Vast right now, so the `E30`, `E34a`, and `E34c` tranches were run under a matched `TORCHDYNAMO_DISABLE=1` eager fallback, and `CAL-01` showed that the current full-run watchdog policy is too aggressive for `8xH100` calibrations because it can kill a run before final exact/export eval. `P-01`, `P-02`, `P-03`, `P-04`, `P-05`, `P-06`, `X-01`, `X-02`, `X-05`, `E00`, `E01`, `E02`, `E24a`, `E27`, `E28`, `E30`, `E32`, `E34a`, `E34c`, and `E35` are done, and the first baseline exporter sweeps `E03`/`E04` are also complete and flat. The Runpod `1xH100` fallback is now proven for proxy work and the Runpod `8xH100` lane is proven for full launches, but the broader repo next tranche is now same-provider baseline control plus proxy-calibration repair plus full-run decomposition rather than another new proxy branch. Detailed closeout review for the Track F experiment branch: [E27](../postmortems/e27_doc_aligned_batching.md), [CAL-01](../postmortems/cal01_full_run_calibration.md), [Proxy Calibration Meta](../postmortems/proxy_calibration_meta.md).
...
- `CAL-01` also exposed a process bug, not just a losing stack: the winning short `E30` proxy never really entered the later large-batch stage, so it implicitly promoted phase 1 of a staged schedule without validating phase 2. That is now a harness rule change, not just a one-off lesson
